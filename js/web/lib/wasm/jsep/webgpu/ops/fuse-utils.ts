...
export const getActivationSnippet =
    (attributes: InternalActivationAttributes, valueType: string, baseType = 'f32', varName = '${varName}'): string => {
      switch (attributes.activation) {
        case 'Relu':
          return `${varName} = max(${varName}, ${valueType}(0.0));`;
        case 'Sigmoid':
          return `${varName} = (${valueType}(1.0) / (${valueType}(1.0) + exp(-${varName})));`;
        case 'Clip':
          return `${varName} = clamp(${varName}, ${valueType}(${baseType}(uniforms.clip_min)), ${valueType}(${
              baseType}(uniforms.clip_max)));`;
        case 'HardSigmoid':
          return `${varName} = max(${valueType}(0.0), min(${valueType}(1.0), ${baseType}(uniforms.alpha) * ${
              varName} + ${baseType}(uniforms.beta)));`;
        case 'LeakyRelu':
          return `${varName} = select(${baseType}(uniforms.alpha) * ${varName}, ${varName}, ${varName} >= ${
              valueType}(0.0));`;
        case '':
          return '';
